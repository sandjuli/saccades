...
% % the code is assigning each trial to be either a "vowels" or a
% %   "consonants" one (-> these are the conditions.) -> this is saved in
% %   either a value of "1" or of "2" which is (I assume) then used when
% %   detemrining trial outcome and generating feedback. Scope this.
%
% % we will have 4 x 72 unique stimuli-> each will be assigned to one
% % condition and divided into 2 sets of 36 (8 overall)
%
% % then, the "first" sets of 36 for each condition will be presented in a random order,
% %   followed by the second sets
...
% rand_folder = randperm(4,4); % randomly select 4 folders from 3
% for i = 1:4
%
%     folder_name = strcat('set_',num2str(rand_folder(i)),'/');
%     folder_1{i} = folder_name; % set folder of images
% end
...
%condition = {'Do not count As (Eyes freely moving)','Count all (Eyes freely moving)','Do not count As (Eyes fixed)','Count all (Eyes fixed)'};
...
% % Load Image
% instruction_start = Screen('MakeTexture',window,imread('instructions/instructions_start.tiff'));
% instruction_end = Screen('MakeTexture',window,imread('instructions/instructions_end.tiff'));
% instruction_start2 = Screen('MakeTexture',window,imread('instructions/instructions_start_2.tiff'));
% instruction_startexp = Screen('MakeTexture',window,imread('instructions/instructions_startexp.tiff'));
...
% instruction_start2_text = 'What to expect for the task\n\nIn the tasks, you will need to count the number of\neither ALL objects or all objects EXCEPT "A"s\npresent in the image.\nYou will know which one to count before the image\n appears.\n\n You will either do this freely moving \n your eyes, or focussing on a red "x" in\nthe middle of the screen.\nYou will know whether to focus on the "x" or not\nat the beginning of each block of trials.\n\nImportant: once you feel like you\nknow the answer, you can press spacebar.\nThis will take you to the response screen\n right away. Stimuli are presented for 2s max.\nPress any key to continue.'; % Define your instruction text
...
% %Display
% Screen('DrawTexture',window,instruction_start,[],InsSq);
% Screen('Flip', window);
% KbStrokeWait;
%
% Screen('DrawTexture',window,instruction_start2,[],InsSq);
% Screen('Flip', window);
% KbStrokeWait;
...
%     myFiles=table2struct(myFilestempsorted);
    ...
    while notdone && trial < num_trial+1
        ...
        % convert code so that its text and not images containing the
        % instructions.
            ...
            if fixation_condition
                ...
                if look_away(trial)
                    ...
                    if first_time_poss_wrong
                        ...
                        % need to make sure that the same image is appended.
                        % Also, that the corresponding condition is added at the
                        % the relevant position in the the final_condition_order
                        % so that the conditions make sense.
                    ...
                    cont_var = 1;
                    %                     continue; % skip to the next iteration of the trial for loop

                    % does this just "restart" the while loop? ! %%%%%%%%%%
                    % seems like it (restarts response function)
                end
            end
